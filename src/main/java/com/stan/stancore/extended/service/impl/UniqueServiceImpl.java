...
import com.systemspecs.remita.vending.vendingcommon.service.ProcessorUniqueService;
...
public class UniqueServiceImpl implements UniqueService {
    ...
    private static final String ADMIN_REQUEST_ACCOUNT = ">>> AdminUniqueRequest account: {}";
    ...
    @Override
    public ResponseEntity<AdminResponse> performCustomerEnquiry(AdminUniqueRequest request) {
        ...
        ProcessorUniqueService concreteService = adminPortalFactory.getProcessorUniqueService(processorId);
        if (Objects.isNull(vendingServices) || Objects.isNull(concreteService)) {
            log.info(PROCESSOR_NOT_FOUND);
            return ResponseEntity.ok(response);
        }
        try {
            AdminResponse result = concreteService.customerEnquiry(request);
            if (result == null) {
    ...
    @Override
    public ResponseEntity<AdminResponse> performShiftEnquiry(AdminUniqueRequest request) {
        log.info(ADMIN_REQUEST_ACCOUNT, request);
        AdminResponse response = new AdminResponse();
        response.setCode(TransactionStatus.TRANSACTION_FAILED.getCode());
        response.setMessage(TransactionStatus.TRANSACTION_FAILED.getMessage());
        ...
        try {
            AdminResponse result = concreteService.shiftEnquiry(request);
            if (result == null) {
                throw new NotFoundException(ACCOUNT_NOT_FOUND);
            }
            if (!result.getCode().equalsIgnoreCase(ResponseStatus.SUCCESS.getCode())) {
    ...
    @Override
    public ResponseEntity<AdminResponse> perfomVendorInformation(AdminUniqueRequest request) {
        log.info(ADMIN_REQUEST_ACCOUNT, request);
        AdminResponse response = new AdminResponse();
        response.setCode(TransactionStatus.TRANSACTION_FAILED.getCode());
        response.setMessage(TransactionStatus.TRANSACTION_FAILED.getMessage());
        ...
        try {
            AdminResponse result = concreteService.vendorInformation(request);
            if (result == null) {
                throw new NotFoundException(ACCOUNT_NOT_FOUND);
            }
            if (!result.getCode().equalsIgnoreCase(ResponseStatus.SUCCESS.getCode())) {
    ...
    @Override
    public ResponseEntity<AdminResponse> performCriteriaType(AdminUniqueRequest request) {
        log.info(ADMIN_REQUEST_ACCOUNT, request);
        AdminResponse response = new AdminResponse();
        response.setCode(TransactionStatus.TRANSACTION_FAILED.getCode());
        response.setMessage(TransactionStatus.TRANSACTION_FAILED.getMessage());
        ...
        try {
            AdminResponse result = concreteService.criteriaType(request);
            if (result == null) {
                throw new NotFoundException(ACCOUNT_NOT_FOUND);
            }
            if (!result.getCode().equalsIgnoreCase(ResponseStatus.SUCCESS.getCode())) {
